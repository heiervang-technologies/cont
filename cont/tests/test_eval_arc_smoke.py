"""Smoke tests for ``cont.teach.eval_arc_smoke`` — no daemon, no GPU.
...
- The integration with :mod:`cont.teach.eval` — when ``arc_agi_3`` is
...
from cont.teach import eval as eval_mod
from cont.teach import eval_arc_smoke
...
def test_eval_dispatch_invokes_custom_runner(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    ``run()`` also appends the standing chuddite knowledge gate on every
    invocation, and that gate is a custom_runner too — so patching the
    resolver globally means the dummy is called a second time with n=1.
    Both invocations are asserted rather than filtered out: if the gate
    ever stops running, this test should notice.
    """
    ...
    # arc_agi_3 dispatched with the daemon URL stripped of /v1, then the
    # standing chuddite gate at n=1.
    assert invocations == [
        {"daemon_url": "http://127.0.0.1:8768", "n": 2},
        {"daemon_url": "http://127.0.0.1:8768", "n": 1},
    ]
    assert [t.task for t in result.tasks] == ["arc_agi_3", "chuddite_gate"]
    tr = result.tasks[0]
...
def test_arc_agi_3_registered_with_custom_runner_spec() -> None:
    ...
    assert meta["custom_runner"] == "cont.teach.eval_arc_smoke:run_arc_eval"
