...
Run: ``pytest cont/tests/test_rlvr_loop.py -xvs``
"""
...
from cont.teach import rlvr_loop
from cont.teach.rlvr_loop import (
    RLVRConfig,
    RLVRScheduler,
    build_combined_spec,
)
...
def test_scheduler_dry_run_skips_submit(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cfg = RLVRConfig(k=4, source="math", log_path="/tmp/trainfer_rlvr_loop_test.jsonl")
...
def test_scheduler_submits_when_not_dry_run(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    cfg = RLVRConfig(
        k=2, source="math", log_path="/tmp/trainfer_rlvr_loop_test_submit.jsonl"
    )
