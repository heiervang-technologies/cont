"""pytest conftest for the cont test suite.

Most of this suite is stdlib + urllib — the harness and the teachers talk
to a running trainfer daemon over HTTP rather than importing it. A few
files (``test_rlvr_loop.py``) do import ``trainfer.controller`` /
``trainfer.state`` at module scope and therefore need torch. On a runner
without torch we fall back to a whitelist of files known to import
without it.
...
cpu_only CI bucket. The check raises at conftest load so the miss is loud,
not a silent skip.

(Inherited from the daemon-side conftest in ``trainfer``. The two are
deliberately independent copies — the repos share no Python imports, and
copying beats coupling for ~100 lines of collection policy.)
"""
...
# ``cont/tests/`` will be skipped at collection time when torch is absent.
_TORCHLESS_OK = {
    "test_eval_harness.py",  # harness smoke — urllib-only
    "test_eval_arc_smoke.py",  # async CLI over stdlib HTTP; no torch import
    "test_eval_chuddite.py",  # scoring gate — pure string work
...
def _ast_has_cpu_only_pytestmark(tree: ast.AST) -> bool:
    ...
    Matches the shapes found in the suite today:
...
def _cpu_only_whitelist_violations(tests_dir: Path, whitelist: set[str]) -> list[str]:
    ...
    *and* import cleanly in the current (torchless) interpreter.
    ...
    module scope is stdlib-only but whose test bodies lazy-import torch
    will pass the probe and still fail at test-run time. That's a
    misclassification bug in the test file itself (the ``cpu_only`` marker
    is lying); the fix is to drop the marker or make the dep available to
    the cpu_only CI env, not to extend this probe.
    """
    violations: list[str] = []
    for p in sorted(tests_dir.glob("test_*.py")):
        ...
        if not _ast_has_cpu_only_pytestmark(tree):
            continue
        spec = importlib.util.spec_from_file_location(f"_cont_whitelist_probe_{p.stem}", p)
...
if _missing("torch"):
    ...
    if _violations:
        raise AssertionError(
            "cont/tests/conftest.py: _TORCHLESS_OK is out of sync with the "
    ...
    # torch is available but unsloth is not — common for quick-iteration CI
    # that wants the cpu_only suite without the heavy unsloth git+ dep.
    here = Path(__file__).parent
