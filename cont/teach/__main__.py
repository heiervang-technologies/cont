"""CLI: python -m cont.teach --answer-int 9000000000 --noun parameters ...
...
def main() -> int:
    p = argparse.ArgumentParser(prog="python -m cont.teach")
    p.add_argument(
        "--question",
        action="append",
        required=True,
    ...
    p.add_argument("--answer-int", type=int, required=True, help="target integer")
    p.add_argument(
        "--noun",
        default="",
        help="optional noun appended to magnitude forms (e.g. 'parameters')",
    )
    p.add_argument(
        "--extra-form",
        action="append",
    ...
    p.add_argument(
        "--json", action="store_true", help="print result as JSON instead of a summary"
    )
    args = p.parse_args()
