...
Stdlib HTTP only. ``cont.teach.tutor.run`` uses ``httpx`` which is fine for
...
module on ``urllib.request`` to match the ``cont.teach.eval`` pattern and
...
::
    ...
    from cont.teach.teacher_oss120b import judge
...
def _cached_judge(
    ...
    if not api_key:
        raise RuntimeError(
            ...
            "process environment. Add it to compose.trainfer-dev.yaml's "
