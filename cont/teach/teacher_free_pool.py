...
free-tier models configured in ``cont/teach/free_models.json``. The pool
...
When the entire remote pool exhausts (or the env var ``TRAINFER_TEACHER_LOCAL_ONLY``
is set), the dispatcher falls back to the local cluster's gemma-4-31b-iq4
endpoint (read from ``$TRAINFER_LOCAL_TEACHER_URL``). The local backend is
...
::
    ...
    from cont.teach.teacher_free_pool import judge
    ...
    async with httpx.AsyncClient(timeout=60.0) as c:
        r = await c.post(
            ...
            headers={
                ...
                "X-Title": "trainfer-rlvr-teacher-pool",
            },
            json=payload,
        )
    r.raise_for_status()
...
async def _call_local(model: str, body_prompt: str, *, max_tokens: int = 800) -> str:
    url = os.environ.get("TRAINFER_LOCAL_TEACHER_URL")
    if not url:
        raise RuntimeError(
            "TRAINFER_LOCAL_TEACHER_URL not set — no local fallback configured"
        )
    payload = {
        "model": model,
        "messages": [{"role": "user", "content": body_prompt}],
...
async def judge(
    ...
    *,
    ...
    if not os.environ.get("TRAINFER_TEACHER_LOCAL_ONLY"):
