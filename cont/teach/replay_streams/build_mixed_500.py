...
def cold_generate(prompt: str, domain: str, endpoint: str, seed: int) -> str:
    ...
    3. POST `{endpoint}/chat/completions` with
       ...
       — the trainfer server ignores the model name and uses the live weights.
...
def main() -> int:
    p = argparse.ArgumentParser(
        prog="python -m cont.teach.replay_streams.build_mixed_500",
    )
    p.add_argument(
        "--endpoint",
    ...
    p.add_argument("--out", default="cont/teach/replay_streams/mixed_500.jsonl")
