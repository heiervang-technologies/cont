"""Offline eval harness for trainfer — regression-check at n=100 per task.

CLI entry point for the harness specified in
`docs/research/eval-harness.md`. Hits the OpenAI-compatible endpoint
of a running trainfer daemon (or any server that speaks /v1/chat/completions)
...
`commit_cursor` for A/B reproducibility.
    ...
    uv run python -m cont.teach.eval \\
        ...
        --limit 100 \\
        --out data/evals/baseline.json

Deps are opt-in via the `cont[eval]` extra (lm-eval, evalplus). Running
...
LM_EVAL_TASKS: dict[str, dict[str, Any]] = {
    ...
    "arc_agi_3": {
        "metric": "acc",
        "custom_runner": "cont.teach.eval_arc_smoke:run_arc_eval",
    },
    ...
    "chuddite_gate": {
        "metric": "pass_rate",
        "custom_runner": "cont.teach.eval_chuddite:run_chuddite_gate",
    },
}
...
# ----------------------------------------------------------------- runners
def _run_lm_eval(
    task: str, endpoint: str, model: str, limit: int, batch_size: int
) -> TaskResult:
    ...
    value = float(
        results.get(meta["metric"], results.get(f"{meta['metric']},none", 0.0))
    )
    return TaskResult(
        task=task,
        metric=meta["metric"],
...
def _run_evalplus(task: str, endpoint: str, model: str, limit: int) -> TaskResult:
...
def run(
    ...
    for task in tasks:
        if task not in LM_EVAL_TASKS:
            raise SystemExit(
                f"unknown lm-eval task: {task!r} (known: {sorted(LM_EVAL_TASKS)})"
            )
    ...
    for task in code_tasks:
        if task not in CODE_TASKS:
            raise SystemExit(
                f"unknown code task: {task!r} (known: {sorted(CODE_TASKS)})"
            )
        print(f"[eval] {task} (n={limit})", file=sys.stderr)
    ...
    return RunResult(
        ...
        endpoint=endpoint,
        model=model,
        commit_cursor_before=cursor_before,
        commit_cursor_after=cursor_after,
        tasks=results,
    )
        ...
        "tasks": [
            {
                "task": t.task,
                "metric": t.metric,
                "value": _jsonable_value(t.value),
                "n": t.n,
                "wall_clock_s": round(t.wall_clock_s, 2),
                "stub": t.stub,
                "raw": t.raw,
            }
...
def main() -> int:
    p = argparse.ArgumentParser(prog="python -m cont.teach.eval")
    p.add_argument(
        "--endpoint",
        default="http://127.0.0.1:8768/v1",
        help="OpenAI-compatible endpoint root (include /v1)",
    )
    p.add_argument("--model", required=True, help="Model name passed to the endpoint")
    p.add_argument(
        "--tasks",
        default="hellaswag,arc_easy,arc_challenge,gsm8k",
    ...
    result = run(
        args.endpoint, args.model, lm_tasks, code_tasks, args.limit, args.batch_size
    )
