...
Pipeline:
  ...
  4. For each `split=eval` prompt, capture a **pre-training** student
     generation via the trainfer daemon's /v1/chat/completions.
...
Usage:
    OPENROUTER_API_KEY=... \\
    .venv/bin/python -m cont.teach.tutor.run \\
        --prompts cont/teach/tutor/seed_prompts.jsonl \\
        --daemon http://127.0.0.1:8766 \\
        --tutor openai/gpt-oss-120b \\
        --out data/tutor_run_01
...
def call_tutor(
    ...
    r = client.post(
        ...
        headers={
            ...
            "X-Title": "trainfer tutor distillation",
        },
        json={
            "model": model,
            "messages": [
...
def main() -> int:
    p = argparse.ArgumentParser(prog="python -m cont.teach.tutor.run")
