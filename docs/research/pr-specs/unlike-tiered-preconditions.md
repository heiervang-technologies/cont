...
Land after #15 (kl_anchor target-position scope). Single-file change (`trainfer/objectives/unlike.py`) + new test cases appended to `test_unlike_loss.py`. Zero schema change; `allow_unanchored` is a new optional key that defaults to `False`.
