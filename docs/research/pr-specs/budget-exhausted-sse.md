...
Piggybacks on `trainfer/commit_stream.py::CommitBroadcaster`. Adds one new broadcast method + one new consumer branch.
...
- **No per-user/session scoping.** The daemon is single-tenant from a trajectory perspective; multi-user budgets are out of scope for trainfer v2.
