...
New file: `trainfer/objectives/safety.py` with `safety_monitor_loss()` following the same signature shape as `kl_anchor_loss`. Returns `(loss=0.0 tensor, components_dict)`. The zero-loss tensor is required to keep the composition grammar consistent with other batch objectives.

Dispatch: register in `trainfer/objectives/__init__.py` under `"safety_monitor"`.
...
Single PR: new `trainfer/objectives/safety.py` + registry entry + `test_safety_monitor.py` + one smoke_objectives step. No route changes. No schema migration.
