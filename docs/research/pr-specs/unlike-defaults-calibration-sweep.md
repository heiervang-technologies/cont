...
`cont/teach/rlaif/calibration_corpus.jsonl`, ~20 hand-picked prefix entries covering the household-AI correction classes:
...
`cont/teach/rlaif/neutral_corpus.jsonl`, ~20 prefixes where the bad tokens from the calibration corpus are NOT expected to be plausible candidates. Used to measure false-fire rate — trigger should almost never fire on these.
...
`docs/research/unlike-defaults-calibration.md` with:
...
The η default in `trainfer/config.py` is updated to match Cleo-A's lower bound (or the empirical safe-η threshold if A hasn't landed yet). This is a separate small PR, gated on the sweep completing.

## Tests

N/A — this is a research sweep, not a unit-testable component. The sweep script itself is a smoke-runnable module (`cont/teach/rlaif/run_calibration.py`) that any contributor can re-run.

## Rollout

1. Land `cont/teach/rlaif/calibration_corpus.jsonl` and `neutral_corpus.jsonl` (hand-curated).
2. Land `cont/teach/rlaif/run_calibration.py` (sweep driver).
3. Run on free GPU (~2h).
4. Commit `docs/research/unlike-defaults-calibration.md` with the results.
