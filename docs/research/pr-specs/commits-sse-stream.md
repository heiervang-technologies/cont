...
**Date:** 2026-04-18
**Status:** proposed, awaiting implementer (codex / trainfer-backend)
...
2. **RLAIF real-time feedback loop.** `cont/teach/rlaif/run.py` (in-flight, architect track) needs low-latency commit confirmation to issue the next tutor-as-critic batch. Polling `/v1/state/stats` is wrong-grained (coarse + expensive); SSE is the right primitive.
...
- Ship as a new route in a `trainfer/routes/stream.py`. No changes to existing routes, zero back-compat risk.
