...
**Actual current verifier shapes** (verified by reading `trainfer/objectives/verifiers/_math.py` and `_code.py`):
...
- `trainfer/ttrl_mv.py` — PR #34 implementation; extension points are the stat buckets in `__init__` and the vote-to-submit path in `_maybe_run_one`.
- `trainfer/objectives/verifiers/` — existing verifier plugin point; L.1 extends the protocol rather than replacing it.
- Razin-safety framing (`trainfer/GLOSSARY.md`) — already correctly applied in PR #34; none of the follow-ups change the safety class.
