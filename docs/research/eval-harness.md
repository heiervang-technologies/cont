# trainfer — offline eval harness (research-only)
...
trainfer already serves OpenAI-compatible `/v1/chat/completions`. The harness treats the running daemon as the model-under-test and never loads the LoRA adapter itself. Consequences:
...
- Works with whichever backend (trainfer daemon on :8768, studio backend on :8888, external vLLM/llama.cpp) happens to speak the same API
...
```
uv run python -m cont.teach.eval \
  ...
  --batch-size 16 \
  --limit 250 \
  --out data/evals/<run_id>.json
```

- `--limit 250` for research iteration; full eval when promoting a PR.
- `--endpoint` points at whatever is serving the OpenAI API. No trainfer-specific deps.
...
New extras group `trainfer[eval]` in `trainfer/pyproject.toml`:
...
2. Apply a canned 500-event feedback stream (`cont/teach/replay_streams/mixed_500.jsonl` — TBD) to populate state
...
2. For any task that regresses by >T pp, pull a canonical training batch for that skill from `cont/teach/rehearsal/<task>.jsonl` (fixed, curated, small — ~50 samples per skill).
...
1. lm-eval-harness's `local-chat-completions` needs a chat template. Does the trainfer daemon apply its own or does it expect raw strings? (Need to verify — server.py line 123 onwards.) If daemon applies template, harness must pass messages, not a flattened prompt. This is a one-line flag.
...
**Capability dimensions chart.** A Studio page (`/trainfer/capabilities` or a tab on the existing `/trainfer` page) showing each dimension over time. Dimensions and their ~100-sample validation sets:
...
- [x] Land `cont/teach/eval.py` (~150 LOC) with the four tasks wired
- [x] `trainfer[eval]` extra in `pyproject.toml` + CI smoke on the stub path
...
- [ ] Document in `trainfer/README.md` how to run locally
- [ ] Live Qwen3-0.6B + Qwen3-9B baselines committed to `docs/research/baselines/` (needs a daemon run; CI scope is stub-only)
