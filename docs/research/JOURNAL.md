...
**Result:**
<numbers + plot link or path under data/research/<slug>/>
...
**Setup:**
- Daemon: Qwen3-8B-unsloth-bnb-4bit / max_seq=4096 / lora_rank=16 / data_dir=data
...
Plot: `data/research/R001/retention_curves.png` (all 3 segments)
Data: `data/research/R001/results.raw.jsonl` (3 headers + 219 loop records total; raw
...
**Setup:**
- Daemon: Qwen3-8B-unsloth-bnb-4bit / max_seq=4096 / lora_rank=16 / data_dir=data
- Generator: `cont/teach/research_fixtures/mythical_facts.py` seed=42, 100 facts
...
Data: `data/research/R001/results.jsonl` (clean 101-line file: 1 header + 100 loop
...
Data: `data/research/R004/results.jsonl`
...
- Daemon: Qwen3-8B-unsloth-bnb-4bit / max_seq=4096 / lora_rank=16 / data_dir=data
- Generator: `cont/teach/research_fixtures/mythical_facts.py` seed=42, 100 facts
- Runner: `./cont/teach/research/r001_memorize_retention.py` with new CLI flags
...
|-----|:-:|:--:|:-:|:-:|---|
| 1 (strong) | 10 | 100 | 5e-4 | 0.95 | `data/research/R001b/results.jsonl` |
| 2 (low-thresh) | 3 | 30 | default | 0.70 | `data/research/R001b_thresh07/results.jsonl` |
...
the trainfer training surface, scored on the continual-learning composite
...
**Setup:**
- Daemon: `unsloth/Qwen3-8B-unsloth-bnb-4bit` / max_seq 4096 / LoRA r=16 / `~/ht/agi/lile_data` (pre-split paths; now `data/` in this repo)
- `TRAINFER_IDLE_REPLAY=0`, `autoload_on_boot=False` (after exp-006 found idle_replay was
...
- Snapshot bracket: `autoresearch_baseline` saved at experiment start, restored at end.
  ...
  same model state.
- Corpus: `trainfer/objectives/verifiers/corpora/logical/tasks_v0.json` 30 tasks across 10 domains, 20 train / 10
...
1. **A clean continual-learning recipe**: K=5 memorize + CoT eval gives
   ...
   For trainfer's product use case (live-learning local daemon, persistent
...
*is* a reasonable default for the trainfer product when context-token cost
