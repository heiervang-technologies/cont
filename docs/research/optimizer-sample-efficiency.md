# trainfer — optimizer & sample-efficiency research

Status: draft. Owner: mei (%30, SVP of AI Research). Scope-partner: claude-opus (%4, replay/forgetting/per-sample slice + eval harness).

Seed issue: [#7 "trainfer: revisit optimizer choice for online-learning daemon"](https://github.com/heiervang-technologies/ht-unsloth/issues/7).
...
Gradient magnitudes across registered objectives (`OBJECTIVES` in `trainfer/objectives/__init__.py`):
...
`Controller.request_snapshot_load` in `trainfer/controller.py:433-436` calls `snapshots.load(...)` via the queue but does not touch `TrainEngine._opt`. After a load, `m` and `v` are EMAs of gradients taken against weights that no longer exist — for ~τ_m ≈ 10 steps of `m` and ~τ_v ≈ 1000 steps of `v`, updates are subtly mis-scaled. Model recovers after ~10 steps, but those ~10 steps produce silently degraded deltas.
...
`cont/tests/test_per_objective_param_groups.py`:
...
2. The harness is being extended into a **skill-targeted re-teach loop** — on regression detection, a small canonical rehearsal batch (`cont/teach/rehearsal/<task>.jsonl`, ~50 samples per skill) gets pushed through `/v1/train` and the model is re-evaluated. This converts the harness from a gate into a closed-loop rehearsal mechanism.
...
4. **8B maintainer-triggered eval — resolved.** CI on 0.6B always. Pre-merge 8B block only for PRs touching `trainfer/engine/`, `trainfer/objectives/`, `trainfer/state.py`, `trainfer/controller.py`. Codify as a GitHub Actions path filter when wiring CI. PR B, C, and D all fall under the 8B block; PR A does too (touches `controller.py`).
...
- [Issue #7 — trainfer: revisit optimizer choice for online-learning daemon](https://github.com/heiervang-technologies/ht-unsloth/issues/7) (seed)
- [`docs/research/eval-harness.md`](eval-harness.md) (PO's eval slice)
- [`docs/research/sample-efficiency-synthesis.md`](sample-efficiency-synthesis.md) (PO's synthesis across all three failure modes)
- [`trainfer/STATUS.md`](../../STATUS.md) and [`trainfer/DESIGN.md`](../../DESIGN.md) (current state + invariants)
