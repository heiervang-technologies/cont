...
- **Scope**: EMNLP 2025 main + findings; filtered against `trainfer` citations.
...
- **Why it matters**: exact "sparse feedback from live users" regime `trainfer` operates in — empirical warning that naive implicit-feedback SFT is noisy; motivates IW-SFT + KL-guarded objectives.
...
- **Why it matters**: direct per-user LoRA analog of `trainfer`'s regime; benchmarks are reusable eval harnesses.
...
- **Why it matters**: a non-training baseline `trainfer` must beat. Ablation target: "does LoRA delta outperform decode-time steering at N=100?"
...
- **Why it matters**: informs `trainfer`'s `replay_streams`; cold-start results argue for a hybrid retrieval-plus-adapter stance.
...
- Relevant if `trainfer` ever federates per-user daemons.
...
- **Why it matters**: cheap gradient-based valuation as a priority signal for `trainfer`'s live queue — lighter baseline vs LESS and DataInf.
...
- **Why it matters**: inverse of the usual "filter the noise" heuristic — argues `trainfer` should up-weight low-confidence feedback, not drop it. Testable queue-admission policy.
...
- Relevant if `trainfer` ever trains multiple objectives concurrently under memory pressure.
...
- **Why it matters**: CoH/Constitutional-AI descendant producing *structured* critiques; ACU is a reasonable unit for `trainfer`'s critique-and-revise objective.
...
- **Why it matters**: `trainfer`'s critique channel may not need full NL rationales — a simple "this was wrong" + context could suffice. Strong motivation for an ablation.
...
- **Why it matters**: EMNLP 2025's cleanest RLVR-for-instruction-following recipe. Verifier-construction blueprint if `trainfer` adds an RL phase.
