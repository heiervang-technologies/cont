...
- **Why it matters for `trainfer`**: this is almost literally the daemon's loop — a model deciding how to turn a feedback event into a LoRA update. Mine the RL-on-edits formulation and the forgetting mitigation notes.
...
- **Why it matters for `trainfer`**: direct blueprint for reward-model-free online RL when explicit user feedback is sparse. Majority-vote surrogate could fill the 10–100 req/hr gaps.
...
- **Why it matters**: curriculum mechanism for picking which stored feedback events to replay next — maps onto `trainfer`'s replay stream design.
...
- **Why it matters**: direct replacement/upgrade for `trainfer`'s current `replay_streams`. DER++ is already cited in the lit review; SuRe is the 2025 evolution.
...
- **Why it matters**: current `trainfer` objective list stops at KTO/IPO; RePO is the 2025 addition worth evaluating for pairwise user-feedback events.
...
- **Why it matters**: directly lowers the compute floor for `trainfer`'s GRPO path — our exact regime (sparse prompts, expensive rollouts).
...
- **Why it matters**: lightweight alternative to full PPO/GRPO for the conversational-feedback shape — candidate objective for `trainfer/objectives/`.

---

## Synthesis

Must-reads: **SEAL** (architectural twin of `trainfer`) and **SuRe** (immediate upgrade path for replay). **TTRL + CoVo** together cover the reward-model-free regime when feedback is silent. **RePO** belongs on the preference-objective short list alongside existing KTO/IPO. **Intruder-dim mitigation** from "Illusion of Equivalence" is a mechanical fix that can ship as a diagnostic first, mitigation second. **Nested Learning / Hope** is a longer-horizon bet — read it for the frame, not a direct port.
