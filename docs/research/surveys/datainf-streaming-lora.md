...
- **Limitation for `trainfer`**: designed for a batch of training examples against a fixed validation set — no streaming API.
...
- **Limitation for `trainfer`**: pre-computed features on a static dataset.
...
1. Existing influence methods assume a fixed training set. `trainfer` has an append-only trajectory.
2. Existing methods rank full gradients against a fixed validation set. `trainfer` needs a *per-feedback-event* estimate of "should this train step happen at all, and with what weight?"
...
A ~100 LOC experimental probe at `cont/teach/datainf_probe.py`:
