...
- **Why it matters**: directly justifies `trainfer`'s option to put nonzero weight on prompt/context tokens; simpler knob than Rho-1 excess-loss gating.
...
- **Why it matters**: cheap on-policy filter for the trainfer replay buffer — skip teaching the model things it already says correctly.
...
- **Why it matters**: fits `trainfer`'s selective-loss story with a principled DPO interpretation; complementary to SFT-GO.
...
- **Why it matters**: directly informs `trainfer`'s default mixture weights when user feedback is sparse — the daemon's exact regime.
...
- **Why it matters**: CoH-adjacent; gives a clean SFT-only data recipe `trainfer` can adopt without adding an RL stage.
...
- Heavier than `trainfer`'s target; good pointer for when users explicitly provide revision feedback.
...
- **Why it matters**: direct successor to O-LoRA; plausible drop-in `trainfer` objective alongside CLoRA's A/B regularizer — both cheap, buffer-free.
...
- **Why it matters**: simplest-possible regularizer for task-free continual SFT; implementable as a `trainfer` optimizer flag in ~20 lines.
...
- Architecturally heavy for `trainfer`'s single-user daemon; relevant if we later support per-task adapter stacks.
...
- **Why it matters**: closest published match to `trainfer`'s core design — online preferences + anti-forgetting in one objective.
...
- **Why it matters**: blueprint for `trainfer`'s future multi-user mode.
...
- **Why it matters**: near-identical problem framing to `trainfer` (sequential conversations, per-user LoRA, weighted CE). Worth a close read for data-augmentation ideas.
...
- **Why it matters**: activation-steering as complement to `trainfer`'s gradient-based correction — near-instant behavior fixes before a training step has converged.
...
- **Balancing the Budget** is empirical gold for defending `trainfer`'s SFT-heavy default at low feedback counts.
