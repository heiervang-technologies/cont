...
- **Why it matters**: closest first-order meta-learning result to `trainfer`'s regime. Suggests periodically treating the replay buffer as a task distribution and running a Reptile outer loop over per-user-cluster inner loops.
...
### COLD-Steer `[SKIP]`
- Training-free activation steering that approximates GD on in-context examples (2025). Inference-only, so nothing actionable for `trainfer`.
...
- **Why it matters**: if `trainfer` can condition on `(user_id, recent_feedback_embedding)`, a hypernetwork could emit the per-session LoRA delta without SGD.
...
- **Why it matters**: the "meta-initialization + fast per-user adapt" pattern `trainfer` was built for.
...
- **Why it matters**: maps cleanly onto `trainfer`'s multi-user stream if users cluster by style/domain.
...
- **Why it matters**: validates the "RL from implicit signals at inference" pattern `trainfer` targets.
...
- **Why it matters**: concrete warning for `trainfer`'s SFT-heavy default. Testable claim.
...
- **Why it matters**: directly applicable as a per-user reward-modeling head on top of `trainfer`'s KTO/hinge objectives.
...
1. **Meta-initialization is a real lever.** Meta-LoRA, ReptiLoRA, MRM, FSPO all show a pre-training meta-learning phase over users/tasks produces a LoRA init that adapts in tens of steps — plausibly `trainfer`'s cold-start-per-user story.
...
4. **TTRL + TTC-RL's warning** (SFT on traces hurts TTT) is a concrete, testable claim against `trainfer`'s SFT-heavy default and belongs on the eval harness.
