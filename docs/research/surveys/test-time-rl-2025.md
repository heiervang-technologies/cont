...
Prepared for Mei (SVP AI Research). Scope: 2025-2026 literature on reasoning training methods relevant to `trainfer`. Overlaps with `sample-efficiency-lit-review.md` §5 (GRPO, V-STaR, ReST-EM, AReaL, AsyncFlow, Asynchronous RLHF) are deliberately skipped.
...
**Implication for trainfer.** Don't build a PRM. GRPO + a clean ORM is the 2026-consensus default and matches what `trainfer/objectives/` is already structured around. If we later add agentic / multi-tool objectives where rollouts diverge early, revisit PRMs then. Consider λ-GRPO as a drop-in upgrade — small algorithmic tweak, reportedly faster convergence.
