...
When 8-bit is unavailable (no CUDA / no bnb install), `trainfer/engine/train.py::_optimizer` falls back to `torch.optim.AdamW`. This is already implemented and tested. Memory cost at LoRA r=16 on Qwen3-8B: ~102 MB per 32-bit state vs ~25 MB for 8-bit — negligible at our peak (8.21 GB measured).
