...
- **Scope**: ICLR 2026 accepted + ICLR 2025 load-bearing; filtered against existing `trainfer` citations.
...
- **Why it matters**: strongest ICLR 2026 argument that live-weight updates at serving time are feasible and architecture-compatible — direct theoretical cover for `trainfer`.
...
- **Why it matters**: cite-magnet framing for "sparse scalar feedback is trainable signal" that matches `trainfer`'s user-feedback loop exactly.
...
- **Why it matters**: per-user/per-task LoRA generation without inner-loop gradients — natural mode for `trainfer`'s identity-LoRA direction.
...
- **Why it matters**: closest 2026 analog to "materialize a user-LoRA on the fly" — contrast point for `trainfer`'s live training.
...
- **Why it matters**: drop-in upgrade over KL-regularized DPO for `trainfer`'s sparse-feedback regime.
...
- **Why it matters**: justifies that `trainfer` gets a handful of signals per user and that's sufficient.
...
- **Why it matters**: exactly the evaluation frame `trainfer` needs — position the daemon's modes on this Pareto.
...
- Foil to `trainfer` — argues the training-free axis; worth citing to defend why weight-space adaptation is still needed.
...
- **Why it matters**: directly informs `trainfer`'s layer-selection and regularization; reframes what "forgetting" even means.
...
- **Why it matters**: architectural template for stacking / rotating adapters as `trainfer` accumulates user sessions.
...
- **Why it matters**: `trainfer` already has KL infrastructure — this is a ready upgrade path, and complements Spurious Forgetting nicely.
