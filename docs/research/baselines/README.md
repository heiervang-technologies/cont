# trainfer eval baselines

Diff-visible regression anchors for `cont/teach/eval.py`. A baseline is a
single JSON file with the shape emitted by `python -m cont.teach.eval
--out …`. When the harness output format changes, the file diff in a PR
makes the change obvious.

## Files

- **`stub.json`** — produced without the `trainfer[eval]` extras installed.
...
- **`qwen3-0_6b.json`** *(TODO)* — n=100 per task on Qwen3-0.6B,
  generated with `uv sync --extra eval` and a running trainfer daemon.
  Commit after the first live run. This is the Qwen3-0.6B CI smoke
  baseline from `docs/research/eval-harness.md §"Baseline choice
  for CI"`.
...
uv run python -m cont.teach.eval \
    ...
    --limit 100 \
    --out docs/research/baselines/stub.json
```

With `trainfer[eval]` uninstalled the tasks fall back to stub entries. Run
...
See `docs/research/eval-harness.md §"A/B protocol"`. The "win"
