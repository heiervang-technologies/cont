...
Three things went wrong with the tutor_run_01 A/Bs before they stabilized. Each failure mode has a cheap, mechanical fix. This doc is the checklist that every future trainfer A/B runs through before a number gets quoted in a finding, a PR, or a memory entry.
...
- Call-sites: `cont/teach/eval.py`, the trajectory_cursor harness, `filter_to_misses` A/B rig.
